...
def get_args():
	...
	parser.add_argument(
		"--host", help="ipaddress/host to lookup", type=str, metavar="ipaddr"
	)
	...
	parser.add_argument(
		"--ipwhois", help="ipwhois lookup", action="store_true", default=False
	)
	parser.add_argument(
		"-vt",
		"--virustotal",
	...
	parser.add_argument(
		"--spam", help="spam lookup", action="store_true", default=False
	)
	parser.add_argument(
		"-abip",
		"--abuseipdb",
		help="abuseipdb lookup",
		action="store_true",
		default=False,
		dest="abuseipdb",
	)
	...
	parser.add_argument(
		"--ftgd_blk",
		help="get ftgd_blk from graylog",
		action="store_true",
		default=False,
		dest="ftgd_blk",
	)
	parser.add_argument(
		"--sslvpnloginfail",
		help="get sslvpnloginfail from graylog",
		action="store_true",
		default=False,
		dest="sslvpnloginfail",
	)
	parser.add_argument(
		"-def",
		"--defender",
		help="search in defender",
		action="store_true",
		default=False,
		dest="defender",
	)
	...
	parser.add_argument(
		"--all", help="use all lookups", action="store_true", default=False
	)
...
async def main(args):
	...
	if args.url:
		...
		try:
			...
			defenderdata = await search_remote_url(
				args.url, token, limit=100, maxdays=3
			)
			...
			print(
				f"{Fore.LIGHTBLUE_EX}vt url info  {Fore.CYAN} {len(vt_url_resultdata)}:{Fore.YELLOW} {vturlinfo.get('data').get('attributes').get('stats')}{Style.RESET_ALL}"
			)
			for vendor in vt_url_resultdata:
				if vt_url_resultdata.get(vendor).get("category") == "malicious":
					print(
						f"{Fore.CYAN}   Vendor: {vendor} result: {vt_url_resultdata.get(vendor).get('result')} method: {vt_url_resultdata.get(vendor).get('method')} {Style.RESET_ALL}"
					)
			print(
				f"{Fore.LIGHTBLUE_EX}defender data:{Fore.YELLOW} {len(defenderdata.get("Results", []))} {Style.RESET_ALL}"
			)
			if len(defenderdata.get("Results", [])) >= 1:
				results = defenderdata.get("Results", [])
				for res in results[: args.maxoutput]:
					print(
						f"{Fore.CYAN}   {res.get('Timestamp')} device: {res.get('DeviceName')} action: {res.get('ActionType')} url: {res.get('RemoteUrl')} user: {res.get('InitiatingProcessAccountName')} {res.get('InitiatingProcessAccountUpn')} {Style.RESET_ALL}"
					)
	...
	if args.urlscanio:
		try:
			...
			if urlscandata:
				print(
					f'{Fore.LIGHTBLUE_EX}urlscanio {Fore.LIGHTBLACK_EX}results:{Fore.YELLOW} {urlscandata.get("total")} '
				)
				for res in urlscandata.get("results"):
					print(
						f"{Fore.CYAN} time: {res.get('task').get('time')} vis: {res.get('task').get('visibility')} url: {res.get('task').get('url')} "
					)
			else:
				logger.warning(
					f"no urlscanio data for {args.host} urlscandata: {urlscandata}"
				)
	...
	if args.vturl:
		...
		vt_url_resultdata = vturlinfo.get("data").get("attributes").get("results")
		print(
			f"{Fore.BLUE}vt url info:  {Fore.GREEN}{len(vt_url_resultdata)}: {vturlinfo.get('data').get('attributes').get('stats')}"
		)
		for vendor in vt_url_resultdata:
			if vt_url_resultdata.get(vendor).get("category") == "malicious":
				print(
					f"{Fore.BLUE}   Vendor: {vendor} {Fore.CYAN}result: {vt_url_resultdata.get(vendor).get('result')} method: {vt_url_resultdata.get(vendor).get('method')} "
				)
			...
			if results.get("hits").get("total").get("value") > 0:
				...
				if "citrixtype" in df.columns or "type" in df.columns:
					if "citrixtype" in df.columns or (df["type"] == "citrixtype").any():
						...
						if (
							"Total_bytes_recv" in df.columns
							and "Total_bytes_send" in df.columns
						):
							total_recv = df["Total_bytes_recv"].sum()
							total_sent = df["Total_bytes_send"].sum()
							print(
								f"  {Fore.CYAN}Total Traffic - Received: {Fore.YELLOW}{total_recv:,} bytes {Fore.CYAN}Sent: {Fore.YELLOW}{total_sent:,} bytes"
							)
						...
						if (
							"SourceAddress" in df.columns
							and "DestinationAddress" in df.columns
						):
							unique_sources = df["SourceAddress"].nunique()
							unique_dests = df["DestinationAddress"].nunique()
							print(
								f"  {Fore.CYAN}Connection Diversity - Unique Sources: {Fore.YELLOW}{unique_sources} {Fore.CYAN}Unique Destinations: {Fore.YELLOW}{unique_dests}"
							)
				...
				print(
					f"{Fore.GREEN}[1] graylog results:{Fore.LIGHTGREEN_EX} {results.get('hits').get('total').get('value')}"
				)
				for res in results.get("hits").get("hits")[: args.maxoutput]:
					...
					if "msgraph" in res_idx:
						print(
							f"{Fore.YELLOW} {res_idx} {res_msg.get('gl2_receive_timestamp')} {res_msg.get('RequestMethod')} {res_msg.get('displayName')} {res_msg.get('IpAddress')} {res_msg.get('dstip')} {res_msg.get('RequestUri')}"
						)
					else:
						print(
							f"{Fore.YELLOW}{res_idx} {Fore.BLUE}ts:{res_msg.get('timestamp')} {Fore.GREEN} country:{res_msg.get('srccountry')} - {res_msg.get('dstcountry')} {Fore.CYAN} action:{res_msg.get('action')} srcip:{res_msg.get('srcip')} dstip:{res_msg.get('dstip')} transip:{res_msg.get('transip')} service: {res_msg.get('service')} url:{res_msg.get('url')} srcname:{res_msg.get('srcname')}"
						)
				...
				if "msg" in df.columns and "srcip" in df.columns:
					...
					try:
						print(
							df.groupby(["action", "msg", "srcip"])["msg"]
							.agg(["count"])
						...
						logger.error(
							f"KeyError: {e} - check graylog data structure. {df.columns}"
						)

					print(f"{Fore.LIGHTBLUE_EX}top 15 actions by dstip:")
					try:
						print(
							df.groupby(["action", "msg", "dstip"])["msg"]
							.agg(["count"])
							.sort_values(by="count", ascending=False)
							.head(15)
						)
					...
					print(
						df.groupby(["action", "type", "subtype", "srcip", "dstip"])[
							"timestamp"
						]
						.agg(["count"])
						.sort_values(by="count", ascending=False)
				...
				if "citrixtype" in df.columns or "request" in df.columns:
					print(
						f"{Fore.LIGHTBLUE_EX}Citrix data found - processing citrixtype column"
					)
				...
				if "msg" in df.columns and "remip" in df.columns:
					...
					try:
						print(
							df.groupby(["action", "msg", "remip", "username"])["msg"]
							.agg(["count"])
							.sort_values(by="count", ascending=False)
							.head(15)
						)
				...
				print(
					f"{Fore.YELLOW}no graylog data ({results.get('hits').get('total').get('value')}) for {Fore.GREEN}{args.host}{Style.RESET_ALL}"
				)
		else:
			print(
				f"{Fore.YELLOW}no graylog results for {Fore.GREEN}{args.host}{Style.RESET_ALL}"
			)
	...
	if args.sslvpnloginfail and args.graylog:
		...
		if results:
			ipaddres_set = set(
				[k.get("message").get("remip") for k in results.get("hits").get("hits")]
			)
			print(
				f"{Fore.LIGHTBLUE_EX}graylog sslvpnloginfail {Fore.CYAN}results: {results.get('hits').get('total').get('value')} ipaddres_set: {len(ipaddres_set)}"
			)
			for res in results.get("hits").get("hits")[: args.maxoutput]:
				print(
					f"{Fore.YELLOW}   {res_msg.get('timestamp')} {res_msg.get('msg')} {res_msg.get('action')} {res_msg.get('user')} {res_msg.get('remip')} {res_msg.get('source')}"
				)
			token = await get_aad_token()
			for addr in ipaddres_set:
				...
				if args.debug:
					logger.debug(
						f'defender returned {len(defenderdata.get("Results"))} ... searching azure logs for {addr}'
					)
				azuredata = await get_azure_signinlogs(addr)
				if args.debug:
					logger.debug(
						f"azure logs returned {len(azuredata)} ... searching azure failed signin logs for {addr}"
					)
				azuredata_f = await get_azure_signinlogs_failed(addr)
				if args.debug:
					logger.debug(
						f"azure failed signin logs returned {len(azuredata_f)} ... searching graylog for {addr}"
					)
				...
				if len(defenderdata.get("Results")) > 0:
					print(
						f'{Fore.LIGHTBLUE_EX}defender found {Fore.YELLOW}{len(defenderdata.get("Results"))} for {Fore.CYAN}{addr}'
					)
					results = defenderdata.get("Results")
					for res in results[: args.maxoutput]:
						print(
							f"{Fore.CYAN}   {res.get('Timestamp')} device: {res.get('DeviceName')} action: {res.get('ActionType')} url: {res.get('RemoteUrl')} user: {res.get('InitiatingProcessAccountName')} {res.get('InitiatingProcessAccountUpn')} "
						)
				if len(azuredata) > 0:
					print(
						f"{Fore.LIGHTBLUE_EX}azure found {Fore.YELLOW}{len(azuredata)}"
					)
				...
				if len(azuredata_f) > 0:
					print(
						f"{Fore.LIGHTBLUE_EX}azure failed signins found {Fore.YELLOW}{len(azuredata_f)}"
					)
	...
	if args.ftgd_blk and args.graylog:
		...
		if results:
			ipaddres_set = set(
				[k.get("message").get("dstip") for k in results.get("hits").get("hits")]
			)
			print(
				f"{Fore.LIGHTBLUE_EX}[2] graylog results:{Fore.YELLOW} {results.get('hits').get('total').get('value')} {Fore.LIGHTBLUE_EX}ipaddres_set:{Fore.YELLOW} {len(ipaddres_set)}"
			)
			token = await get_aad_token()
			...
			for addr in ipaddres_set:
				...
				if len(defenderdata.get("Results")) > 0:
					print(
						f'{Fore.LIGHTBLUE_EX}defender found {Fore.YELLOW} {len(defenderdata.get("Results"))} {Fore.LIGHTBLUE_EX}for{Fore.CYAN} {addr}'
					)
					results = defenderdata.get("Results")
					for res in results[: args.maxoutput]:
						print(
	...
	if args.defender:
		...
		if token:
			...
			if len([k for k in indicators if args.host in str(k.values())]) >= 1:  # type: ignore
				...
				for ind in indx:
					print(
						f'{Fore.RED}indicator found: {Fore.GREEN} {ind.get("title")} {ind.get("description")} {Fore.LIGHTBLUE_EX}type: {ind.get("indicatorType")} action: {ind.get("action")} {Fore.LIGHTGREEN_EX} created by: {ind.get("createdBy")}'
					)
			else:
				print(
					f"{Fore.YELLOW}no indicator found for {Fore.GREEN}{args.host}{Style.RESET_ALL}"
				)
			try:
				maxdays = 1
				limit = 100
