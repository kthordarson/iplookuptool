...
def create_sample_data(size=100, data_type="basic"):
    ...
    if data_type == "network_events":
        ...
        timestamps = [
            (datetime.now() - timedelta(hours=np.random.randint(0, 24))).strftime(
                "%Y-%m-%dT%H:%M:%S.%fZ"
            )
            for _ in range(size)
        ]
        ...
        remote_ips = (
            ["192.168.1." + str(i) for i in range(1, 21)]
        ...
        app_pools = [
            f"{name}-Pool"
            for name in ["Default", "API", "WebService", "Frontend", "Backend"]
        ]
        ...
        return pd.DataFrame(
            {
                ...
                "ActionType": np.random.choice(
                    ["ConnectionSuccess", "ConnectionFailed"], size=size, p=[0.9, 0.1]
                ),
                "RemoteIP": np.random.choice(remote_ips, size=size),
                "RemotePort": np.random.choice(
                    [80, 443, 8080, 9000, 3389, 22, 1433, 3306], size=size
                ),
                "LocalIP": np.random.choice(
                    ["192.168.1." + str(i) for i in range(50, 70)], size=size
                ),
                ...
                "InitiatingProcessCommandLine": np.random.choice(
                    command_lines, size=size
                ),
        ...
        return pd.DataFrame(
            {
                "RemoteIP": np.random.choice(
                    ["192.168.1.1", "10.0.0.1", "172.16.0.1"], size=size
                ),
                "RemotePort": np.random.choice([80, 443, 8080, 9000], size=size),
                "InitiatingProcessFileName": np.random.choice(
                    ["w3wp.exe", "svchost.exe", "cmd.exe"], size=size
                ),
                "InitiatingProcessFileSize": np.random.randint(1000, 50000, size=size),
                "InitiatingProcessCommandLine": np.random.choice(
                    ["cmd /c whoami", "powershell -enc ...", "w3wp.exe -ap ..."],
                    size=size,
                ),
            }
        )
...
def analyze_dataset(df=None, data_type="basic", show_plots=True):
    ...
    is_network_events = all(
        col in df.columns for col in ["$table", "Timestamp", "DeviceId", "ActionType"]
    )
...
def analyze_basic_data(df, show_plots=True):
    ...
    IQR = Q3 - Q1
    outliers = df[
        (df["InitiatingProcessFileSize"] < Q1 - 1.5 * IQR)
        | (df["InitiatingProcessFileSize"] > Q3 + 1.5 * IQR)
    ]
...
def analyze_network_events(df, show_plots=True):
    ...
    if "Timestamp" in df.columns:
        ...
        if show_plots:
            ...
            if not daily_connections.empty and "Day" in daily_connections.columns:
                daily_connections["Day"] = pd.Categorical(
                    daily_connections["Day"], categories=days_order, ordered=True
                )
    ...
    if (
        "InitiatingProcessCommandLine" in df.columns
        and df["InitiatingProcessCommandLine"].str.contains("-ap").any()
    ):
        ...
        for cmd in df["InitiatingProcessCommandLine"]:
            if isinstance(cmd, str) and "-ap" in cmd:
                match = (
                    cmd.split("-ap")[1].split('"')[1]
                    if '"' in cmd.split("-ap")[1]
                    else cmd.split("-ap")[1].strip().split()[0]
                )
...
def show_basic_plots_from_results(results):
    ...
    if not all(key in results for key in ["ip_counts", "port_counts"]):
        print(
            "Error: Results dictionary doesn't have the required keys for plotting basic data"
        )
        return
...
def show_network_events_plots_from_results(results):
    ...
    if "iptype_counts" in results and not results["iptype_counts"].empty:
        ...
        plt.pie(
            iptype_counts,
            labels=iptype_counts.index,
            autopct="%1.1f%%",
            startangle=90,
            shadow=True,
            explode=[0.05] * len(iptype_counts),
        )
        plt.title("Remote IP Types")
        plt.axis("equal")
        plt.show()
...
def show_network_events_plots(df):
    ...
    if "RemotePort" in df.columns:
        ...
        if "Hour" in df.columns:
            ...
            port_hour_pivot = pd.crosstab(
                port_hour_df["Hour"], port_hour_df["RemotePort"]
            )

            plt.figure(figsize=(16, 8))
            sns.heatmap(
                port_hour_pivot, cmap="YlGnBu", annot=True, fmt="d", linewidths=0.5
            )
    ...
    if all(col in df.columns for col in ["InitiatingProcessFileName", "RemotePort"]):
        ...
        process_port_df = df[
            df["InitiatingProcessFileName"].isin(top_processes)
            & df["RemotePort"].isin(top_ports)
        ]

        process_port_pivot = pd.crosstab(
            process_port_df["InitiatingProcessFileName"], process_port_df["RemotePort"]
        )

        plt.figure(figsize=(14, 8))
        sns.heatmap(
            process_port_pivot, cmap="YlGnBu", annot=True, fmt="d", linewidths=0.5
        )
