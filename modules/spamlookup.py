...
async def spam_lookup(ipaddr):
	...
	async def check_blacklist(bl_host):
		...
		try:
			# Run the DNS lookup in a thread pool to avoid blocking
			await asyncio.to_thread(socket.gethostbyname, ip_rev + '.' + bl_host + '.')
			return bl_host  # Return the blacklist name if found
	...
	try:
		...
		for result in results:
			if isinstance(result, Exception):
				logger.error(f'[!] Error in blacklist check: {result} for address {ipaddr}')
				continue
